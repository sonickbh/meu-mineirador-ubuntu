...
sed -i 's/"max-cpu-usage": *[^,]*,/"max-cpu-usage": 65,/' $HOME/c3pool/config.json
...
if [ "$CPU_THREADS" -lt "4" ]; then
  ...
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 65,/' \$HOME/c3pool/config.json"
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 65,/' \$HOME/c3pool/config_background.json"
fi
echo ""
